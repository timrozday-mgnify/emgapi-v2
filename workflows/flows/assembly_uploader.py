...
async def create_study_xml(study_accession, library, output_dir):
    ...
    if EMG_CONFIG.slurm.assembly_uploader_root_dir:
        ...
        launcher = f"{EMG_CONFIG.slurm.assembler_uploader_python_executable} " \
    ...
    try:
        ...
        if 'Make a note of this!' in output_log or 'An accession with this alias already exists in project' in output_log:
            ...
            if new_acc:
                ...
                raise Exception(f"No assigned assembly study accession was found for {study_accession}")
        else:
            raise Exception(f"No assembly study accession was found in uploader log for {study_accession}")
...
async def process_study(study_accession: str, mgnify_study: analyses.models.Study, run_accession: str,
                        assembly_path: str, mgnify_run: analyses.models.Run, upload_folder: str, dry_run: bool):
    ...
    if not check_study_registration_existence(mgnify_study, run_accession):
        ...
        await create_study_xml(study_accession=study_accession,
...
async def prepare_assembly(run_accession: str, study_accession: str, mgnify_assembly: analyses.models.Assembly,
                           assembler: str, assembler_version: str, assembly_path: str, registered_study: str,
                           upload_folder: str):
...
async def submit_assembly_slurm(run_accession: str, manifest: str, mgnify_assembly: analyses.models.Assembly,
                                upload_folder: str, dry_run: bool):
    command = f"java -Xms2G -jar {EMG_CONFIG.slurm.webin_cli_executor} " \
              f"-context=genome " \
...
async def assembly_uploader(study_accession: str, run_accession: str,
    ...
    registered_study = await process_study(study_accession, mgnify_study, run_accession,
                        assembly_path, mgnify_run, upload_folder, dry_run)
    # Create assembly manifest
    manifest = await prepare_assembly(run_accession, study_accession, mgnify_assembly, assembler,
                                      assembler_version, assembly_path, registered_study, upload_folder)
    # Submit assembly with Webin-cli
    await submit_assembly_slurm(run_accession, manifest, mgnify_assembly, upload_folder, dry_run)
