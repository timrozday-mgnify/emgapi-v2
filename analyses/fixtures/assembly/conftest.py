...
def mgnify_assemblies(raw_read_run, raw_reads_mgnify_study, assemblers):
    assembler_metaspades = assemblers.filter(name=mg_models.Assembler.METASPADES).first()
    assembler_megahit = assemblers.filter(name=mg_models.Assembler.MEGAHIT).first()
    # create assembly objects skipping AMPLICON runs
    assembleable_runs = [run for run in raw_read_run if
                         run.experiment_type != mg_models.Run.ExperimentTypes.AMPLICON.value]
    assembly_objects = []
...
def mgnify_assembly_completed(mgnify_assemblies):
    ...
    metaspades_assemblies = mg_models.Assembly.objects.filter(assembler__name="metaspades", run__ena_accessions__in=[run_accession])
